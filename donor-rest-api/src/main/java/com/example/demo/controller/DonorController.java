...
public class DonorController {
	...
	public ResponseEntity<Donor> searchBycode(@PathVariable("areacode") int n){
		return new ResponseEntity<Donor>(service.searchBycode(n),HttpStatus.OK);
	}
	
	@DeleteMapping(path="/delete/{id}")
	public ResponseEntity<String> delete(@PathVariable("id") int n){
		int count =  this.service.delete(n);
	...
	public Flux<String> findBycode(){

	return this.client.get().uri("http://localhost:7010/api/v1/donorcamp").retrieve().bodyToFlux(String.class);
	}
